...
from context_aware.trainer_v2 import ContextAwareTrainer
from utils import AdditionalLossPrinterCallback, ContextContainerCallback, FrozenLayersManager, \
    Seq2SeqDataCollatorWithPaddingAndConvId, compute_metrics, group_params
...
class CustomTrainingArguments(Seq2SeqTrainingArguments):
    ...
    enable_enc_context: bool = field(
        default=False, metadata={"help": "Whether to enable context for encoder."}
    )
    enable_dec_context: bool = field(
        default=False, metadata={"help": "Whether to enable context for decoder."}
    )
...
if __name__ == '__main__':
    ...
    layer_training_manager = FrozenLayersManager(training_args.enc_layers_to_freeze, training_args.dec_layers_to_freeze,
                                                 ...
                                                 callbacks=[
                                                     model.encoder.base_model.encoder.activate_custom_params
                                                 ])
    ...
    context_container = ContextContainerCallback()
    ...
    trainer = ContextAwareTrainer(
        args=training_args,
        model=model,
        callbacks=[layer_training_manager, early_stopping, printing_callback, context_container],
