...
class Seq2SeqLMOutputLosses(Seq2SeqLMOutput):
    ...
    encoder_context_vector: Optional[Tuple[torch.FloatTensor]] = None


@dataclass
class CausalLMOutputWithContext(CausalLMOutput):
    context_vector: Optional[Tuple[torch.FloatTensor]] = None
...
class Wav2Vec2WithContextV1(Wav2Vec2Model):
    ...
    def forward(
            ...
            context_vector: Optional[torch.Tensor],
        ...
        output = super().forward(input_values, attention_mask, mask_time_indices, output_attentions,
                                 ...
                                 return_dict)
        enc_hidden_states_pooled = self.utterance_pool(output.last_hidden_state)
        stacked_repr = torch.stack((enc_hidden_states_pooled, context_vector)).permute(1, 0, 2)
        context_curr = self.context_pool(stacked_repr)
        output.last_hidden_state = torch.hstack((context_curr[:, None, :], output.last_hidden_state))
        return output


class Wav2Vec2WithContextForCTCV1(Wav2Vec2ForCTC):
    def __init__(self, config: Wav2Vec2Config):
        super().__init__(config)

        self.wav2vec2 = Wav2Vec2WithContextV1(config)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            log_probs = nn.functional.log_softmax(logits[:, :-1, ...], dim=-1, dtype=torch.float32).transpose(0, 1)
...
class JointCTCAttentionEncoderDecoderWithContext(JointCTCAttentionEncoderDecoder):
    ...
    def __init__(self,
        ...
        if encoder is None:
            encoder = Wav2Vec2WithContextForCTCV1(config.encoder)
    ...
    def from_encoder_decoder_pretrained(
        ...
        if encoder is None:
            ...
            encoder = Wav2Vec2WithContextForCTCV1.from_pretrained(encoder_pretrained_model_name_or_path,
                                                                  *model_args,
                                                                  **kwargs_encoder)
    ...
    def forward(
        ...
        context_vector = encoder_outputs['context_vector'] if return_dict else encoder_outputs[-1]
        encoder_hidden_states = encoder_outputs['hidden_states'][-1] if return_dict else encoder_outputs[1][-1]
        encoder_hidden_states = torch.hstack([context_vector[:, None, :], encoder_hidden_states])
        ...
        return Seq2SeqLMOutputLosses(
            ...
            encoder_context_vector=context_vector,
