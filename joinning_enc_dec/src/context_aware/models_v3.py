...
from transformers.models.wav2vec2.modeling_wav2vec2 import Wav2Vec2Attention, Wav2Vec2EncoderStableLayerNorm, \
    Wav2Vec2FeedForward, \
...
class Wav2Vec2EncoderLayerStableLayerNormWithContext(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        self.context_combiner = PrevContextCombiner(config)
...
class Wav2Vec2EncoderStableLayerNormWithContext(Wav2Vec2EncoderStableLayerNorm):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [Wav2Vec2EncoderLayerStableLayerNormWithContext(config) for _ in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        for index, layer in enumerate(self.layers):
            ...
            if not skip_the_layer or deepspeed_zero3_is_enabled:
                ...
                if self.gradient_checkpointing and self.training:
                    ...
                    layer_outputs = layer(
                        hidden_states, context_vectors=context_vectors[:, index,
                                                       ...] if context_vectors is not None else context_vectors,
                        attention_mask=attention_mask,
                        output_attentions=output_attentions
                    )
                hidden_states = layer_outputs[0]
                new_context_vectors.append(layer_outputs[1])
        ...
        return BaseModelOutputWithContext(
...
class Wav2Vec2ModelWithContext(Wav2Vec2Model):
    ...
    def forward(
    ...
    ) -> Union[Tuple, Wav2Vec2BaseModelOutputWithContext]:
        ...
        return Wav2Vec2BaseModelOutputWithContext(
            last_hidden_state=hidden_states,
            extract_features=extract_features,
            context_vectors=encoder_outputs.context_vectors,
