...
class Wav2Vec2ForCTCForGen(Wav2Vec2ForCTC):
    def forward(
            self,
        ...
        if not return_dict:
            ...
            output = super().forward(input_values, attention_mask, output_attentions, output_hidden_states, return_dict,
                                     labels)
...
class JointCTCAttentionEncoderDecoder(SpeechEncoderDecoderModel):
    ...
    def __init__(self,
        ...
        if encoder is None:
            encoder = Wav2Vec2ForCTCForGen(config.encoder)
    ...
    def from_encoder_decoder_pretrained(
        ...
        if encoder is None:
            ...
            encoder = Wav2Vec2ForCTCForGen.from_pretrained(encoder_pretrained_model_name_or_path,
                                                     *model_args,
                                                     **kwargs_encoder)
...
class Wav2Vec2ForCTCWithSharedLM(Wav2Vec2ForCTC):
...
#
#         self.decoder_compatible_proj = nn.Linear(self.config.hidden_size, self.config.decoder_hidden_size)
#
#         self.lm_head = nn.Linear(self.config.decoder_hidden_size, config.vocab_size, bias=False)
#
...
#
#         hidden_states = self.decoder_compatible_proj(hidden_states)
#
...
#
#
# class JointCTCAttentionEncoderDecoderSharedHeads(SpeechEncoderDecoderModel):
#     """Custom model for CTC+Attention loss based on the ESPNet architecture"""
#
#     config_class = SpeechEncoderDecoderConfig
#     base_model_prefix = "speech_encoder_decoder"
...
#         super().__init__(config)
#
#         config.encoder.decoder_hidden_size = config.decoder.hidden_size
#         if encoder is None:
#             encoder = Wav2Vec2ForCTCWithSharedLM(config.encoder)
#
#         if decoder is None:
#             decoder = AutoModelForCausalLM.from_config(config.decoder)
#
...
#             # encoder outputs might need to be projected to different dimension for decoder
#             self.enc_to_dec_proj = nn.Linear(self.encoder.config.hidden_size, self.decoder.config.hidden_size)
#             self.enc_to_dec_proj.weight = self.encoder.decoder_compatible_proj.weight
#             self.enc_to_dec_proj.bias = self.encoder.decoder_compatible_proj.bias
#
#         if self.encoder.get_output_embeddings() is not None:
#             raise ValueError(
#                 f"The encoder {self.encoder} should not have a LM Head. Please use a model without LM Head"
...
#                 )
#
#                 encoder_config.architectures = ["Wav2Vec2ForCTCWithSharedLM"]
#                 # encoder_config.prefix = 'wav2vec2'
#                 encoder_config.decoder_hidden_size = decoder_config.hidden_size
#
#                 if encoder_config.is_decoder is True or encoder_config.add_cross_attention is True:
#                     logger.info(
#                         f"Initializing {encoder_pretrained_model_name_or_path} as a encoder model "
...
#                 kwargs_encoder["config"] = encoder_config
#
#             encoder = Wav2Vec2ForCTCWithSharedLM.from_pretrained(encoder_pretrained_model_name_or_path,
#                                                                  *model_args,
#                                                                  **kwargs_encoder)
#
#         decoder = kwargs_decoder.pop("model", None)
